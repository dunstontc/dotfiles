...
" Custom: {{{1
syn keyword rubyFunc puts print

syn match rubyPunctuation ","
syn match rubyOper "[\-.+=]"
syn match rubyLambda "\(->\)"
" syn match rubyLambdaArg "\(->\)\@<=\(\w\+\)" contains=rubyLambda,rubyPunctuation
syn match rubyLambdaArg "\(->\)\@<=\(\w\+\)" contains=rubyLambda,rubyPunctuation
syn match rubyLambdaArg "\(->\s\)\@<=\(\w\+\)\(,\s\)\(\w\+\)" contains=rubyLambda,rubyPunctuation

hi link rubyFunc Function

hi link rubyOper Conditional
hi link rubyLambda Constant
hi link rubyLambdaArg Identifier
